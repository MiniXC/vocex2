...
def get_srmr(
    ...
    for i, ac_ch in enumerate(gt_env):
        ...
        for j, mod_ch in enumerate(mod_out):
            mod_out_frame = segment_axis(
                mod_ch, wLength, overlap=wLength - wInc, end="pad"
            )
    ...
    if (BW > cutoffs[4]) and (BW < cutoffs[5]):
        ...
        Kstar = 8
    result = np.sum(avg_energy[:, :4]) / np.sum(avg_energy[:, 4:Kstar])
    # normalize to [-1, 1]
    result = np.clip(result, 0, 12)
    result = (result - 0) / (12 - 0) * 2 - 1
    return result
